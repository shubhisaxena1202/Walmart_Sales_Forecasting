...
```{r}
feature <- read.csv("G:/Masters/Sem-1/IDMP/Project/features.csv", header = TRUE, na.strings=":", row.names=NULL)
stores <- read.csv("G:/Masters/Sem-1/IDMP/Project/stores.csv", header = TRUE, na.strings=":", row.names=NULL)
test <- read.csv("G:/Masters/Sem-1/IDMP/Project/test.csv", header = TRUE, na.strings=":", row.names=NULL)
train <- read.csv("G:/Masters/Sem-1/IDMP/Project/train.csv", header = TRUE, na.strings=":", row.names=NULL)
```
...
```{r}
  ...
  geom_line(aes(y = Weekly_Sales), color = "darkred",size=1) + 
  geom_line(aes(y = ArimaPredict), color="red", linetype="twodash")+
  geom_line(aes(y = NnetarPredict ), color="blue", linetype="longdash")+
  geom_line(aes(y = xgbPredict), color="black", linetype="dashed")+
  scale_x_date(breaks= seq(as.Date("2012-06-29"),as.Date("2012-10-26"),by="1 months"),date_labels="%b\n%Y") +
  xlab("Forecast horizon")+
  ylab("Weekly sales in dollars in millions") +
  ggtitle("Comparision of all models over the 18 week forecast horizon")
