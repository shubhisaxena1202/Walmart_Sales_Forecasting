...
predict <- forecast(fit,xreg = external_reg2)
#lines(train)
```


```{r}
colnames(predict) <- "Forecast"
```
...
fcast <- forecast(fit, xreg=external_reg, PI=TRUE)
autoplot(fcast)
```
...
```{r}
newxreg = ts(valid[, features], start = c(2012,21),frequency=365.25/7)
external_reg <- as.matrix(valid[,external_reg])
fit <- nnetar(mts, repeats=100, maxit=100, xreg = external_reg)
